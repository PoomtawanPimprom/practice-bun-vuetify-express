          ...
          <v-card>
            ...
            <v-card-text>
              <v-form @submit.prevent="this.createUser()">
                <v-text-field label="First name" placeholder="first name" clearable error-count=""
                  v-model="formData.fname" outlined color>
                </v-text-field>
                <v-text-field label="Last name" placeholder="last name" clearable error-count=""
                  v-model="this.formData.lname" outlined color></v-text-field>
                <v-row>
                  <v-spacer></v-spacer>
      ...
      <v-container class="pa-8">
        <v-row>
          <v-col cols="12" md="6" lg="3" xl="3" v-for="(item, index) in user" :key="index">
            <Card :id="item.id" :height="340" :title="concatString(item.fname, item.lname)"
